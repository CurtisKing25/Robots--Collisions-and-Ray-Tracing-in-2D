...
public class CollisionsMK2Wgraphics extends PApplet {

/*This is based off of CollisionsMK2.pde and contain the graphics I made to put on the project. 
Window collisions and Conrner collisions for one of the squares are working.*/ 
float angle=0;
float OriginX = 100, OriginY = 100;
double horizontalValue;
double verticalValue;
boolean canRotate = true, inWindowX = true, inWindowY = true, inWindow, inSquareX = false, inSquareY = false, inSquare, xRange = false, yRange = false, range = false;
...
Values were changed so that it was 100 in length and width and
a 5th point of the matrix was added so the centre point could be tracked.*/
...
//rate of rotation when the a or d keys are pressed
int rotateAmount = 2;

//The speed that the robot moves at
int speed = 2;
...
public void draw() {
  //Beige background to better highight the robot
  ...
  //centre point of the square extracted
  float xBot=(float)P1.get(0, 4)+OriginX;
  float yBot=(float)P1.get(1, 4)+OriginY;
  
  //function to draw the robot based off the centre point
  draw_bot(xBot,yBot);
  
  //function to draw the squares. Purosely placed after draw_bot as they would be covered by the rectangles that hide the track animation
  drawBlocks(S);
}


public double [][] generateSquares()
{
  int rl = 2;
  int cl = 5;
  int cl2 = cl*4;
...
public void drawBlocks(Matrix P)
{
...
public void draw_quad(Matrix P) {
  ...
  public void draw_bot(float a,float b)
  {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "CollisionsMK2Wgraphics" };
    if (passedArgs != null) {
